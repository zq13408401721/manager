<template>
  <div>
    <div style="text-align: left">
      <Button type="primary" class="btn-tab" @click="selectClassify(1)">视频分类</Button>
      <Button type="primary" class="btn-tab" @click="selectClassify(2)">上传视频</Button>
      <Button type="primary" class="btn-tab" @click="selectClassify(3)">视频列表</Button>
    </div>
    <div> <!--视频分类-->
      <div v-show="currentSelect == 1" style="text-align: left">
      ...
      <div style="margin-left: 10px" v-show="currentSelect == 2">
        <span>上传视频</span>
        <VideoUpload :treeData="ClassifyData"/>
      </div>
      <div style="margin-left: 10px" v-show="currentSelect == 3">
        <span>视频列表</span>
        <VideoList @select="uploadCoverImgComplete"/>
      </div>
...
import VideoUpload from '../widget/WidgetVideoUpload'
import VideoList from '../widget/WidgetVideoList'
...
export default {
  ...
  methods:{
    ...
    createRoot(){
      ...
      addVideoCategory(category_create).then(res => {
    ...
    addCategory(name,pid){
      ...
      addVideoCategory(category).then(res => {
        console.log("addcategory success:"+res)
      }).catch(err => {
    ...
    popWinCB(nodename){
      ...
      if(this.isEditor){
        ...
        updateVideoCategory(category_editor).then(res => {
          console.log("更新节点成功")
    ...
    deleteNode(data){
      ...
      if(node){
        ...
        deleteVideoCategory(option).then(res => {
          ...
          h('Button', {
            ...
            on: {
              ...
              /*click: () => {
                //编辑
                console.log("编辑:"+data);
                this.currentNode = data.title
                this.$refs.popwin.showCustomTextConfirm()
              }*/
            }
          }),
          h('Button', {
